...
height<-read.csv("full_community_2Aug19.csv", na.strings = "")
...
heightCli.crf<-cforest(AdjHeight~temperature+season+active.layer+permafrost+precipitation+rad_May+rad_Aug+VPD_May+VPD_Aug,data=height.complete, controls=cforest_unbiased(ntree=5000))
heightCli.crf
...
library(pdp)
partial(height.crf, pred.var = "rad_May", plot=TRUE, rug=T, ylim=c(1.5,5.2))

####### species models ######

library(party)
setwd("C:/Users/Emily.Holt/Desktop/Height")
height<-read.csv("tham_2Aug19.csv", na.strings = "")
height.complete<-na.omit(height)
height.complete$grazing=as.factor(height.complete$grazing) 
height.complete$fire=as.factor(height.complete$fire)
str(height.complete)
...
##kitchen sink cRF model ##
height.crf<-cforest(height~.,data=height.complete, controls=cforest_unbiased(ntree=5000))
height.crf

## calculation of r2 ##
height.pred<-predict(height.crf, type="response", OOB=TRUE)
residual<-height.complete$height-height.pred
mse<-sum(residual^2)/length(height.complete$height)
pseudo.R2<-1-mse/var(height.complete$height)
n<-21
adj.R2<-1-(1-pseudo.R2)*((length(height.complete$height)-1)/(length(height.complete$height)-n-1))
adj.R2
